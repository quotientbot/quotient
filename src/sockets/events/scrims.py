...
class SocketScrims(Cog):
    ...
    async def on_request__bot_scrim_create(self, u: str, data: dict):
        ...
        if not all(_v):
            return await self.bot.sio.emit(
                "bot_scrim_create__{0}".format(u), SockResponse(ok=False, error=_v[1]).dict()
            )

        await self.bot.sio.emit(
            "bot_scrim_create__{0}".format(u), SockResponse(data={"id": _v[1].id}).dict()
        )
    ...
    async def on_request__bot_scrim_edit(self, u: str, data: dict):
        ...
        if not all(_v):
            return await self.bot.sio.emit(
                "bot_scrim_edit__{0}".format(u), SockResponse(ok=False, error=_v[1]).dict()
            )
