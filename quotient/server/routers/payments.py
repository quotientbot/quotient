...
async def premium_success(request: Request, txnId: str):
    ...
    await PremiumTxn.get(txnid=txnId).update(raw_data=dict(form), completed_at=get_current_time())
    ...
    end_time = guild.premium_end_time + plan.duration if guild.is_premium else get_current_time() + plan.duration
...
async def premium_failed(request: Request, txnId: str):
    ...
    await PremiumTxn.get(txnid=txnId).update(completed_at=get_current_time(), raw_data=dict(form))
